...
def init_db():
    ...
    INSERT OR IGNORE INTO config(clave,valor) VALUES('msg_bazar','Hola {nombre}! Somos importadores directos de articulos para el hogar y bazar. Esta semana tenemos ofertas increibles al por mayor. Le interesaria ver nuestras novedades?');
    INSERT OR IGNORE INTO config(clave,valor) VALUES('msg_ferreteria','Hola {nombre}! Somos importadores mayoristas de herramientas y ferreteria. Precios imbatibles con stock permanente. Le cuento nuestras novedades?');
    INSERT OR IGNORE INTO config(clave,valor) VALUES('msg_electronica','Hola {nombre}! Somos importadores de electronica y accesorios. Cada semana ingresa mercaderia nueva a precios mayoristas. Le interesa recibir nuestras ofertas?');
    INSERT OR IGNORE INTO config(clave,valor) VALUES('msg_generico','Hola {nombre}! Somos importadores directos y queremos ofrecerle articulos al mejor precio mayorista. Podemos contarle nuestras ofertas de esta semana? Muchas gracias!');
    """)
    ...
    for barrio in barrios:
        ...
        try:
            ...
            data=req.get("https://maps.googleapis.com/maps/api/place/textsearch/json",
                params={"query":f"{query} {barrio} Argentina","language":"es","key":api_key},timeout=10).json()
            places=data.get("results",[])
            tok=data.get("next_page_token")
            # Pagina 2 y 3 (hasta 60 por barrio)
            for _ in range(2):
                if tok:
                    time.sleep(2)
            ...
            for p in places:
                ...
                if pid and pid not in seen:
                    ...
                    # Solo agregar si es de Argentina
                    if "Argentina" in addr or "argentina" in addr.lower():
                        results.append({"name":p.get("name",""),"address":addr,"phone":None,
                                        "rating":p.get("rating",0),"place_id":pid,"zona":extraer_zona(addr)})
                    elif not any(x in addr for x in ["México","Mexico","Chile","Uruguay","Brasil","Colombia","Perú","Peru","Bolivia"]):
                        results.append({"name":p.get("name",""),"address":addr,"phone":None,
                                        "rating":p.get("rating",0),"place_id":pid,"zona":extraer_zona(addr)})
        except: pass
    for p in results:
...
def load_config():
    return jsonify({
        "api_key":get_cfg("api_key"),
        "oferta_texto":get_cfg("oferta_texto"),
        "msg_bazar":get_cfg("msg_bazar"),
        "msg_ferreteria":get_cfg("msg_ferreteria"),
        "msg_electronica":get_cfg("msg_electronica"),
        "msg_generico":get_cfg("msg_generico"),
    })
...
def importar_csv():
    ...
    rubro = request.json.get("rubro","Otro")
    if not data: return jsonify({"error":"Sin datos"}),400
    
    con=get_db(); importados=0
    lines = data.strip().split("\n")
    if not lines: return jsonify({"importados":0})
    
    # Detectar encabezados
    header = [h.strip().strip('"').lower() for h in lines[0].split(",")]
    
    # Mapeo de columnas Google Contacts / formato propio
    COL_NOMBRE = next((i for i,h in enumerate(header) if h in ["nombre","first name","name","firstname"]),None)
    COL_APELLIDO = next((i for i,h in enumerate(header) if h in ["apellido","last name","lastname"]),None)
    COL_TEL = next((i for i,h in enumerate(header) if any(x in h for x in ["telefono","phone 1 - value","phone","mobile","celular","tel"])),None)
    COL_DIR = next((i for i,h in enumerate(header) if any(x in h for x in ["direccion","address","domicilio"])),None)
    COL_ZONA = next((i for i,h in enumerate(header) if any(x in h for x in ["zona","ciudad","city","barrio"])),None)
    COL_NOTAS = next((i for i,h in enumerate(header) if any(x in h for x in ["notas","notes","note","observaciones"])),None)
    COL_ORG = next((i for i,h in enumerate(header) if any(x in h for x in ["organization name","organization","empresa","company"])),None)
    
    for line in lines[1:]:
        if not line.strip(): continue
        # Parse CSV respetando comillas
        import csv as csvmod
        row = next(csvmod.reader([line]))
        
        def get_col(idx):
            if idx is None or idx >= len(row): return ""
            return row[idx].strip().strip('"')
        
        nombre = get_col(COL_NOMBRE)
        apellido = get_col(COL_APELLIDO)
        org = get_col(COL_ORG)
        
        # Armar nombre completo
        if apellido: nombre = f"{nombre} {apellido}".strip()
        if not nombre and org: nombre = org
        if not nombre: continue
        
        # Limpiar telefono - formato Google: "+549... ::: +549... ::: +549..."
        tel_raw = get_col(COL_TEL)
        # Tomar solo el primer numero antes de cualquier separador
        for sep in [" ::: "," :: ",":::","::",";"]:
            if sep in tel_raw:
                tel_raw = tel_raw.split(sep)[0]
                break
        tel_raw = tel_raw.strip()
        # Solo digitos y +
        tel = "".join(c for c in tel_raw if c.isdigit() or c=="+")
        if tel.startswith("+"): tel = tel[1:]
        # Limpiar 54 duplicado: 5454... -> 54...
        if tel.startswith("5454"): tel = tel[2:]
        # Convertir 0... a 54...
        if tel.startswith("0"): tel = "54" + tel[1:]
        if tel and not tel.startswith("54"): tel = "54" + tel
        # Si quedo muy largo (>13 digitos) es invalido
        if len(tel) > 13: tel = ""
        
        dir_ = get_col(COL_DIR)
        zona = get_col(COL_ZONA)
        notas = get_col(COL_NOTAS)
        if org and org != nombre: notas = f"Empresa: {org}. {notas}".strip()
        
        # Verificar duplicado por nombre+telefono
        existe = con.execute("SELECT id FROM clientes WHERE nombre=? AND telefono=?",(nombre,tel)).fetchone()
        if not existe:
            cur = con.execute("INSERT INTO clientes(nombre,telefono,direccion,zona,rubro,notas) VALUES(?,?,?,?,?,?)",
                (nombre,tel,dir_,zona,rubro,notas))
            con.execute("INSERT INTO historial(cliente_id,tipo,detalle) VALUES(?,?,?)",
                (cur.lastrowid,"Alta","Importado desde CSV"))
            importados+=1
