...
def importar_csv():
    ...
    for line in lines[1:]:
        ...
        # Limpiar telefono - tomar solo el primero si hay varios separados por :::
        tel_raw = get_col(COL_TEL)
        tel_raw = tel_raw.split(":::")[0].split(" ::: ")[0].strip()
        tel = "".join(c for c in tel_raw if c.isdigit() or c=="+")
        if tel.startswith("+"): tel = tel[1:]
        # Validar longitud maxima (max 13 digitos para Argentina)
        if len(tel) > 13: tel = tel[:13]
        if tel.startswith("0"): tel = "54" + tel[1:]
        if tel and not tel.startswith("54"): tel = "54" + tel
        # Si el numero es muy largo, descartar
        if len(tel) > 15: tel = ""
